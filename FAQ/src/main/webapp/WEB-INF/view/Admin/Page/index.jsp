...
<rapid:override name="content">
	<blockquote class="layui-elem-quote">
		<span class="layui-breadcrumb" lay-separator="/"> <a
			href="/admin">首页</a> <a><cite>页面列表</cite></a>
		</span>
	</blockquote>
				...
				<c:forEach items="${pageCustomList}" var="p">
					<c:if test="${p.pageStatus!=2}">
						<tr>
							...
							<td>${p.pageKey}</td>
							<td>${p.pageTitle}</td>
							<td><a href="/${p.pageKey}" target="_blank">
									${fn:substring(p.pageContent, 0,20 )} </a></td>
							<td><c:choose>
							...
							<td><a href="/admin/page/edit/${p.pageId}"
								class="layui-btn layui-btn-mini">编辑</a> <a
								href="/admin/page/delete/${p.pageId}"
	...
	<blockquote class="layui-elem-quote layui-quote-nm">
		温馨提示： <br> 1、自定义的页面，无法删除，别名已写入控制器
	</blockquote>
</rapid:override>
