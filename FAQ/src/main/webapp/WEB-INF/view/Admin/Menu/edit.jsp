...
<rapid:override name="content">
	...
	<blockquote class="layui-elem-quote">
		<span class="layui-breadcrumb" lay-separator="/"> <a
			href="/admin">首页</a> <a href="/admin/menu">菜单内容列表</a> <a><cite>编辑菜单内容</cite></a>
		</span>
	</blockquote>
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						名称 <span style="color: #FF5722;">*</span> <input type="text"
							name="menuName" value="${menuCustom.menuName}" autocomplete="off"
							class="layui-input" required>
					</div>
					<br>
					<div class="layui-input-block">
						URL <span style="color: #FF5722;">*</span> <input type="text"
							name="menuUrl" value="${menuCustom.menuUrl}" autocomplete="off"
							class="layui-input" required>
					</div>
					<br>
					<div class="layui-input-block">
						图标 <input type="text" name="menuIcon"
							value="${menuCustom.menuIcon}" layui-input="" class="layui-input">
					</div>
					<br>
					<div class="layui-input-block">
						菜单位置 <select name="menuLevel" id="">
							<option value="1"
								<c:if test="${menuCustom.menuLevel==1}">selected</c:if>>顶部菜单</option>
							<option value="2"
								<c:if test="${menuCustom.menuLevel==2}">selected</c:if>>主要菜单</option>
						</select>
					</div>
					<br>
					<div class="layui-input-block">
						Order <input type="number" name="menuOrder"
							value="${menuCustom.menuOrder}" layui-input=""
							class="layui-input" min="0" max="10" required>
					</div>
					<br>

					<div class="layui-input-block">
						状态 <br> <input type="radio" name="menuStatus" value="1"
							title="显示"
							<c:if test="${menuCustom.menuStatus==1}">checked</c:if>>
						<input type="radio" name="menuStatus" value="0" title="隐藏"
							<c:if test="${menuCustom.menuStatus==0}">checked</c:if>>
					</div>
					<br>
					<div class="layui-input-block">
								...
								<c:forEach items="${menuCustomList}" var="m">
									<c:if test="${m.menuLevel==1}">
										<tr>
											...
											<td><a href="${m.menuUrl}" target="_blank">${m.menuUrl}</a>
											</td>
											<td>${m.menuOrder}</td>
											<td><c:choose>
											...
											<td><a href="/admin/menu/edit/${m.menuId}"
												class="layui-btn layui-btn-mini">编辑</a> <a
												href="/admin/menu/delete/${m.menuId}"
