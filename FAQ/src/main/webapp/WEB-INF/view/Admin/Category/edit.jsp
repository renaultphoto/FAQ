...
<rapid:override name="content">
	...
	<blockquote class="layui-elem-quote">
		<span class="layui-breadcrumb" lay-separator="/"> <a
			href="/admin">首页</a> <a href="/admin/category">分类列表</a> <a><cite>编辑分类</cite></a>
		</span>
	</blockquote>
	<div class="layui-row">
		<div class="layui-col-md4" style="border: 1px solid #FF5722;">
			<form class="layui-form" method="post" id="myForm"
				action="/admin/category/editSubmit">
				<div class="layui-form-item">
					<div class="layui-input-block">
						<strong>修改分类</strong>
					</div>
					<input type="hidden" name="categoryId"
						value="${categoryCustom.categoryId}">
					<div class="layui-input-block">
						名称 <span style="color: #FF5722;">*</span> <input type="text"
							name="categoryName" value="${categoryCustom.categoryName}"
					...
					<div class="layui-input-block">
						父节点 <span style="color: #FF5722;">*</span> <select
							...
							<option value="0"
								<c:if test="${categoryCustom.categoryPid==0}">selected</c:if>>无</option>
						</select>
					</div>
					<br>
					<div class="layui-input-block">
						分类描述 <input type="text" name="categoryDescription"
							value="${categoryCustom.categoryDescription}" class="layui-input">
					</div>
					<br>
					<div class="layui-input-block">
						图标样式 <input type="text" name="categoryIcon"
							value="${categoryCustom.categoryIcon}" class="layui-input">
					</div>
					<br>
					<div class="layui-input-block">
						状态 <br> <input type="radio" name="categoryStatus" value="1"
							title="显示"
							<c:if test="${categoryCustom.categoryStatus==1}">checked</c:if>>
						<input type="radio" name="categoryStatus" value="0" title="隐藏"
							<c:if test="${categoryCustom.categoryStatus==0}">checked</c:if>>
					</div>
					<br>
					<div class="layui-input-block">
					...
					<c:forEach items="${categoryCustomList}" var="c">
						<c:if test="${c.categoryPid==0}">
							<tr>
								...
								<td><a href="/admin/category/edit/${c.categoryId}"
							...
							<c:forEach items="${categoryCustomList}" var="c2">
								<c:if test="${c2.categoryPid==c.categoryId}">
									<tr>
										...
										<td>${c2.categoryStatus}</td>
										<td><a href="/admin/category/edit/${c2.categoryId}"
											class="layui-btn layui-btn-mini">编辑</a> <c:if
