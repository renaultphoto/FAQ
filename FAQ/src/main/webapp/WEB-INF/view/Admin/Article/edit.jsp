...
<rapid:override name="content">
	<blockquote class="layui-elem-quote">
		<span class="layui-breadcrumb" lay-separator="/"> <a
			href="/admin">首页</a> <a href="/admin/article">文章列表</a> <a><cite>修改文章</cite></a>
		</span>
	</blockquote>
	...
	<form class="layui-form" method="post" id="myForm"
		action="/admin/article/editSubmit">
		<input type="hidden" name="articleId"
			value="${articleCustom.articleId}">
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<input type="text" name="articleTitle" lay-verify="title" id="title"
					value="${articleCustom.articleTitle}" class="layui-input">
			</div>
		</div>
		...
		<div class="layui-form-item layui-form-text">
			...
			<div class="layui-input-block">
				<textarea class="layui-textarea layui-hide" name="articleContent"
					id="content">${articleCustom.articleContent}</textarea>
			</div>
		</div>
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-inline">
				<select name="articleChildCategoryId" id="articleChildCategoryId"
					lay-filter="articleChildCategoryId">
					<c:forEach items="${categoryCustomList}" var="c">
						<c:if
							test="${c.categoryPid==articleCustom.articleParentCategoryId}">
							<option value="${c.categoryId}"
								<c:if test="${articleCustom.articleChildCategoryId==c.categoryId}">selected</c:if>>${c.categoryName}</option>
						</c:if>
					</c:forEach>
				</select>
			</div>
		...
		<div class="layui-form-item" pane="">
			...
			<c:set var="tagIds"
				value="${fn:split(articleCustom.articleTagIds,',')}" />
			<div class="layui-input-block">
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-inline">
				<input type="number" name="articleOrder"
					value="${articleCustom.articleOrder}" autocomplete="off"
					class="layui-input">
			</div>
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<input type="radio" name="articleStatus" value="1" title="发布"
					<c:if test="${articleCustom.articleStatus==1}">checked</c:if>>
				<input type="radio" name="articleStatus" value="0" title="草稿"
					<c:if test="${articleCustom.articleStatus==0}">checked</c:if>>
			</div>
		</div>
		<div class="layui-form-item">
			<div class="layui-input-block">
	...
	<blockquote class="layui-elem-quote layui-quote-nm">
		温馨提示： 1、插入代码前，可以点击 <a href="http://liuyanzhao.com/code-highlight.html"
