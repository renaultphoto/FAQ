...
<rapid:override name="breadcrumb">
	...
	<nav class="breadcrumb">
		...
		</a> <i class="fa fa-angle-right"></i> 站点地图 <i class="fa fa-angle-right"></i>
		正文
	</nav>
	<%--面包屑导航 end--%>
</rapid:override>
					...
					<ul>
						<c:forEach items="${articleList}" var="a">
							<li style="padding: 5px"><a
								href="/article/${a.articleCustom.articleId}"
								title="ajax实现form表单提交" target="_blank">${a.articleCustom.articleTitle}</a>
							</li>
						</c:forEach>
					...
					<ul>
						<c:forEach items="${categoryList}" var="c">
							<c:if test="${c.categoryPid==0}">
								<li class="cat-item" style="padding: 5px"><a
									href="/category/${c.categoryId}">丨- ${c.categoryName}</a></li>
								<ul class="children">
									<c:forEach items="${categoryList}" var="c2">
										<c:if test="${c2.categoryPid==c.categoryId}">
											<li class="cat-item" style="padding: 5px 30px;"><a
												href="/category/${c2.categoryId}" target="_blank">${c2.categoryName}</a>
											</li>
										</c:if>
									</c:forEach>
								</ul>
			...
			<div class="layui-colla-item">
				...
				<div class="layui-colla-content layui-show">
					<c:forEach items="${tagCustomList}" var="t">
						<a href="/tag/${t.tagId}"
							style="font-size: ${t.articleCount/4+14}px"
